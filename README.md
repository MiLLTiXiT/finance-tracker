...
| **Recurring** | Recurring bills (name, category, amount, **Frequency**, due day, active checkbox, **Paid** checkbox) with an annual projection and monthly/annual totals. **Frequency** drives the Weekly Budget: **Monthly** (full amount in its due‑day week), **Weekly** (full amount every week), or **Monthly split** (the monthly amount spread evenly across weeks — amount ÷ 4.33 reserved each week). Due Day only applies to Monthly items. **Paid**: tick it when you've paid this period's bill and it drops out of the Weekly Budget; an unpaid bill keeps counting (rolls over) until you tick it — untick when the next period's bill comes due. |
| **Goals** | Two sections. **① Weekly Budget** (top): your **Cash on hand** (spendable Cash accounts) minus the **unpaid recurring bills due on/before this Saturday** = **Safe to spend** — how much you can spend for the rest of the week without missing a bill. An itemized **"This week's bills"** list shows each bill still owed with its date and amount. Unpaid bills roll over week‑to‑week until marked Paid on the Recurring tab. **② Savings Goals** (below): target amount & date, saved so far, monthly contribution, with computed remaining, % complete, months left and an on-track flag. |
