...
| **Recurring** | Recurring bills (name, category, amount, **Frequency**, due day, active checkbox) with an annual projection and monthly/annual totals. **Frequency** drives the Weekly Budget: **Monthly** (full amount in its due‑day week), **Weekly** (full amount every week), or **Monthly split** (the monthly amount spread evenly across weeks — amount ÷ 4.33 reserved each week). Due Day only applies to Monthly items. |
| **Goals** | Two sections. **① Weekly Budget** (top): your **Cash on hand** (spendable Cash accounts) minus the **recurring bills still due this week** = **Safe to spend** — how much you can spend for the rest of the week without missing a bill (bills already past this week are treated as paid). Uses the due days on the Recurring tab. **② Savings Goals** (below): target amount & date, saved so far, monthly contribution, with computed remaining, % complete, months left and an on-track flag. |
